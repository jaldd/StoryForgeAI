...
> 回归基线：`cli/` 下 `python -m pytest tests/ -q` = 3 存量失败（`test_strip_polisher_meta`/`test_runtime_dir_override`/`test_defaults`）+ 全绿，零新增。
...
- [ ] **T2 prompts.py：load_recent_human 纯函数**
  改动：新增 `load_recent_human(dir_path, n=3, slice_chars=1000, progress=print) -> str`
...
- [ ] **T3 prompts.py + agent.py + cli.py：滚动注入接线（writer 专属）**
...
- [ ] **T4 prompts.py：de-AI prompt 构造函数**
...
- [ ] **T5 agent.py + state.py：_fix_spans 抽共用 + deai_refine**
...
- [ ] **T6 cli.py：_do_write 的 de-AI 判定链（D4 时序）**
...
- [ ] **T8 harness.py：run_tests 的 deai 留痕断言**
...
- [ ] **T9 `.env copy.example` + ROADMAP：配置注释与状态**
