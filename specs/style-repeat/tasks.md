...
前置：design.md 已定稿（D1-D9 / Z1-Z6 拍板口径）。
约定：每个任务单文件单职责，完成后即跑「验证」栏；全程测试不联网（tmp 目录 + FakeLLM + monkeypatch）。
...
- [ ] **T2 checker.py：run_cross_checks**
...
- [ ] **T3 agent.py：_checker 接线（cross issues 进既有闭环）**
  改动：`NovelAgent.__init__` 增 `recent_endings: Optional[list[str]] = None`（注入模式同
  `recent_human` 先例；None = 跳过）。`_checker`（`agent.py:532-559`）：`run_checks` 之后，
...
- [ ] **T4 prompts.py + cli.py：负面清单注入**
  ...
  4 个调用点同改，`compile_syntax_patterns` 在此预编译，RuntimeError 走命令层既有捕获，
  Z4）；`_do_status` 增「跨章禁则」行（参照章数 / 禁用收束句条数 / 句式配额条数或未配置，
  C12）。
...
- [ ] **T6 设计文档同步：`.env copy.example` 无改动确认 + 规则模板**
...
5. `cli/` 下 `python -m pytest tests/ -q` 零新增失败（3 存量失败除外）。
