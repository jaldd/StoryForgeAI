...
> 状态：**执行中**（2026-08-28 规划，同日代码复核修订一轮；2026-08-29 阶段 0 完成；2026-08-30 `quality-gate`（1.1-1.4）实现完成并通过真车验收（kimi-k3 精修一章全链路：样文路由/checker/八维审稿/fixer/评分门禁），`exemplar-routing`（样文按任务精选注入，1.3/1.5 相关）落地，思考模型兼容层（max_tokens 翻倍重试/温度 400 锁定/reasoning_effort 分流/温度全量可配）就绪；`style-loop`（1.5-1.6）spec 三件套已建，实现待开发）
...
| 阶段 | 主题 | 一句话 | 状态 |
|------|------|--------|------|
| 0 | 修硬伤+快赢 | 修三个地基问题 + 局部精修/接线/清理/模型可换/流水线补漏 | 完成（0.1-0.8 全部真实验收：glm-5.3 精修、kimi-k3 写作双源跑通） |
| 1 | 质量闭环 | 审核维度化 + 学人味 + 去 AI 味，质量能被度量、被门禁 | 进行中（1.1-1.4 `quality-gate` 已实现+真车验收；1.5-1.6 `style-loop` 已实现，真车验收待跑） |
| 2 | 吞吐 | 流式输出 + 批量连写 | 实现完成（2.1 流式 + 2.2 章纲批量已落地，461 测试全绿；真车验收待跑，含 NOVEL_STREAM=1 ark 网关流式形态冒烟） |
...
- **现状**：只有"去 AI 味"的治疗思路，没有"学人味"的预防思路。`正文/` 下的人工正文随写作增长，是比静态 exemplar 丰富得多、且动态的文风样本，目前完全没被用（`index rebuild` 排除整个正文目录）。
...
> 状态：**实现完成，真车验收待跑**（2026-08-31；spec 见 `specs/throughput/`，W1-W7 落地，W8 校准等 P1 见 tasks.md。真车门槛：NOVEL_STREAM=1 冒烟一章确认 ark 网关流式形态，不兼容则 NOVEL_STREAM=0 一行回退。）
