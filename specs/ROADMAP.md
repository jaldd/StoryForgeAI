...
> 状态：**讨论稿**（2026-08-28 规划讨论产物，未开工；同日经代码复核修订一轮）
...
| 阶段 | 主题 | 一句话 | 状态 |
|------|------|--------|------|
| 0 | 修硬伤+快赢 | 修三个地基问题 + 局部精修/接线/清理/模型可换/流水线补漏 | 进行中（0.1-0.6 已完成，剩 0.7/0.8） |
| 1 | 质量闭环 | 审核维度化 + 学人味 + 去 AI 味，质量能被度量、被门禁 | 未开工 |
| 2 | 吞吐 | 流式输出 + 批量连写 | 未开工 |
...
## 阶段 1：质量闭环（核心 feature：`quality-loop`）
...
- **打回路由：不做**。维持现有 `_reject_target` 单一目标机制，feedback 里带维度信息指路（如"文风/AI 味问题=只改表达不动剧情"）。按维度路由到不同 agent 是泛化 `_reject_target` 的小步、不是新机制，但收益存疑（writer 重写后本来就会重过 polisher），先不做，等数据说话。
- **打回机制待定**：现状打回=从零重写（好稿陪葬、round 2 换新毛病）。1.1 设计时定修复式打回（保住 draft、按维度意见只改问题处）vs 维持重写。
...
- **阈值校准**：默认阈值先在既有 run 记录上回测误伤率再上线（run 日志都在，回测零成本）；阶段 2 批量开闸前复核一次。
...
1. **词表格式**：`NOVEL_DIR` 下的词表/阈值用 md 还是 json？跟 0.2 的铁律文件是否合并成一个"写作规则包"？
2. **批量连写默认节奏**：默认每章过目（推荐）还是默认全自动？
3. **阶段 1 拆分**：质量闭环 6 项是一个 feature 一次做完，还是拆两个（审核维度化+硬 gate 先行，学人味+去 AI 后行）？
