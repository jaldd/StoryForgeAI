...
> 遵循 `.specify/memory/constitution.md`；结论沿用 `specs/ROADMAP.md` 条目 0.4。

---

## 1. 目标回顾

新增 `改 <文件>` 命令：段落编号列表 -> 选段 -> 选区级润色（跳过 reviewer）-> 区间整块回填（其余逐字节不变）-> diff + y/n 确认 -> 存回 + 重新入索引。产出可复用的「选区+上下文 -> 改写 -> 回填」管线，供 ROADMAP 1.6 复用。
...
**结论**：只想改一小段时，现状至少付 2 次全文调用（润色+审稿），最坏 4 次（+好坏对比+评测）；且每次整章覆盖存回都有把好段润坏的风险（#6/#7）。ROADMAP 0.4 的痛点成立。
...
ROADMAP 的「单次调用」理解为**选区级而非整章级**（相对整章流水线 5-6 次调用）。对不连续选段（`3,7`）：
...
与 ROADMAP 门禁总原则一致：机器拿不准时交给人，不 fail-closed（REPL 有人）。
...
对 ROADMAP 0.4 三条验收逐项核对：改 3-5 段后其余段落逐字节不变；`---` 与章节头保留；差异展示只含选区。

---

## 9. 开放问题（实现中发现的相邻问题，不实施）

1. **整章 `精修` 的 `_is_better` 也是全文调用**（`cli.py:53-69`）：强制定稿分支的对比与 `evaluate` 的全文评测都不在局部通道内，整章命令的 token 问题依旧。属 ROADMAP 阶段 1（门禁统一 1.4）范畴。
...
## 10. 异议（对 ROADMAP 0.4 结论的复核）

**无异议。** 逐条复核结论均可直接沿用：「跳过 reviewer，人就是门禁」「按段落区间整块回填」「选区级 token」「复用管线供 1.6」均成立且与本设计一致。唯一需要解释口径的是「单次调用」对不连续多区间的含义（见 §5.3，已按「每区间一次」设计并列入开放决策，不改变 ROADMAP 的 token 预算意图）。
...
| # | 决策点 | 本设计采用 | 备选 |
|---|--------|-----------|------|
| D1 | 命令名 | `改 <文件>`（ROADMAP 原文） | `局部精修` / `改段` |
| D2 | 选段语法 | `3` / `3-5` / `3,7` / `3-5,7`（组合支持） | 仅 ROADMAP 示例三式，不组合 |
