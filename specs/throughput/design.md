...
| # | 决策 | 理由 |
|---|---|---|
| R1 | 流式输出 + 批量连写为一个 feature（throughput） | 早期规划阶段 2 整体定义 |
| R2 | 批量默认每章过目确认 | 阶段 2.2 规划原文（防跑偏累积，人在环 = 1.4 门禁天然兜底） |
| R3 | `--auto` 必须先定义低分章去向 | 阶段 1.4 规划推论（前置设计题） |
| R4 | 章间自动更新工作记忆 + 入库 | 阶段 2.2 规划原文 |
